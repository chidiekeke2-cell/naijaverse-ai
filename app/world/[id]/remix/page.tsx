...
export default function RemixPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const res = await fetch(`/api/worlds/${id}/remix`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ instruction }) });
  ...
  return <main className="page-shell"><section className="container narrow section-pad">
    <Link href={`/world/${id}`} className="back-link"><ArrowLeft size={16}/> Back to world</Link>
    ...
    {result && <div className="result-panel"><div className="eyebrow">New version</div><h2>{result.title}</h2><p>{result.description}</p><Link href={`/play/${id}`} className="button button-small">Play remix <ArrowRight size={15}/></Link></div>}
  </section></main>;
}
